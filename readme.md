...
In today's post, I am going to look at our conferencing system that we built to use with our customers. 

This is part 1 of a new series and when we are finish, we will have the following functionality in our application:
...
At Sinch we support both regular phone calling and WebRTC calling for conferencing as well as one-on-one conversations. When I built this system I started out from the [NET Conference Calling template](https://github.com/sinch/net-ConferenceCalling) and went from there. The complete solution can be downloaded [here](https://github.com/sinch/net-demo.sinch.com).

![conference calling startpage](images/startpage.png)

## Using the system
You can just deploy it to Azure using the button below, it will set up a database and configure Sinch app keys and secrets for you, or just fork it and deploy where ever you want. 
...
The system consists of 3 main parts and they all have some small sub features. The callback functionality manages both data and phone calls to connect them to the correct conference. 
...
The third part its the actual front end UI where we display information about connecting to the conference and also the WebRTC browser option for the conference. Let’s start by looking at the Callback functionality.

### Callback API
To enable you to control what happens when someone is dialling a Sinch number, you need to configure a callback URL in your app, on the same screen where you add the numbers.

[![call back settings sinch](images/dashboard.png)](https://www.sinch.com/dashboard#/apps)

Now that this is out of the way, what we want to do is to enable both regular phones and Web Browsers to join a conference. The [ICE callback ](https://www.sinch.com/docs/voice/rest/#callbackapi) information can help us in customizing the functionality for each use case. 

1. Calling in from a phone, we want to play a prompt to the user to enter the conference PIN in order to be connected
2. When calling from a browser, we just want to present a PIN code window even before they attempt to call, and then connect browser caller with no further interaction
...
That’s really all there is to it for the backend. I am sure you you are curious about the conference Model now, so let’s look at the Conference creation. I decided that for creating a conference I wanted a protect login using ASP.NET identity, which is nothing fancy and you can read about how to use it [here](http://www.asp.net/identity).

## Creating a Conference 
So to have a more "real" conference system I wanted set an expiry time for the conference and some basic info like owner of the conference. All of the fields will make sense when we look at the creation of the conference page. 
...
![creating a conference call](images/createconference.png)
...
So now we have a conference, I took the decision to not allow the user to set the PIN themselves because I felt it was a better user experience to not fail on duplicate codes. 
...
```
One other cool feature on details is you can call out to a phone number to add them to the conference. 
...
Now we actually have real life Conference system with some security using a PIN. In the next tutorial, we will add a WebClient to call in to a conference. 
